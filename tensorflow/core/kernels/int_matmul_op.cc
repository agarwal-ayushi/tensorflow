...
#include "tensorflow/core/kernels/int_matmul_op.h"
...
template <typename Device, typename T, bool USE_CUBLAS>
class IntMatMulOp : public OpKernel {
 public:
  explicit IntMatMulOp(OpKernelConstruction* ctx) : OpKernel(ctx) {
...
struct MatMulFunctor<CPUDevice, T> {
  void operator()(
      ...
      const Eigen::array<Eigen::IndexPair<Eigen::DenseIndex>, 1>& dim_pair) {
    IntMatMul<CPUDevice>(d, out, in0, in1, dim_pair);
  }
};
...
struct MatMulFunctor<SYCLDevice, T> {
  void operator()(
      ...
      typename MatMulTypes<T>::in_type in1,
      const Eigen::array<Eigen::IndexPair<Eigen::DenseIndex>, 1>& dim_pair) {
    IntMatMul<SYCLDevice>(d, out, in0, in1, dim_pair);
  }
};
#endif  // TENSORFLOW_USE_SYCL
...
#define REGISTER_CPU(T)                                                        \
  REGISTER_KERNEL_BUILDER(                                                     \
      Name("IntMatMul").Device(DEVICE_CPU).TypeConstraint<T>("T"),                \
      IntMatMulOp<CPUDevice, T, false /* cublas, ignored for CPU */>);            \
  REGISTER_KERNEL_BUILDER(                                                     \
      Name("IntMatMul").Device(DEVICE_CPU).TypeConstraint<T>("T").Label("eigen"), \
      IntMatMulOp<CPUDevice, T, false /* cublas, ignored for CPU */>)

#define REGISTER_GPU(T)                                            \
  REGISTER_KERNEL_BUILDER(                                         \
      Name("IntMatMul").Device(DEVICE_GPU).TypeConstraint<T>("T"),    \
      IntMatMulOp<GPUDevice, T, true /* cublas, true by default */>); \
  REGISTER_KERNEL_BUILDER(Name("IntMatMul")                           \
                              .Device(DEVICE_GPU)                  \
                              .TypeConstraint<T>("T")              \
                              .Label("cublas"),                    \
                          IntMatMulOp<GPUDevice, T, true /* cublas */>)
...
#define REGISTER_SYCL(T)                                         \
  REGISTER_KERNEL_BUILDER(                                       \
      Name("IntMatMul").Device(DEVICE_SYCL).TypeConstraint<T>("T"), \
      IntMatMulOp<SYCLDevice, T, false /* xxblas */>);              \
  REGISTER_KERNEL_BUILDER(Name("IntMatMul")                         \
                              .Device(DEVICE_SYCL)               \
                              .TypeConstraint<T>("T")            \
                              .Label("eigen"),                   \
